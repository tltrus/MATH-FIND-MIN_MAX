...
namespace WPFSurfacePlot3D
{
    public class SurfacePlotVisual3D : ModelVisual3D
    {
        ...
        /// <summary>
        /// Gets or sets the collection of points to display as spheres
        /// </summary>
        public CustomPoint3DCollection Points
        {
            get { return (CustomPoint3DCollection)GetValue(PointsProperty); }
            set { SetValue(PointsProperty, value); }
        }
        public static readonly DependencyProperty PointsProperty = DependencyProperty.Register("Points", typeof(CustomPoint3DCollection), typeof(SurfacePlotVisual3D), new UIPropertyMetadata(null, ModelWasChanged));
        ...
        private Model3DGroup CreateModel()
        {
            ...
            double pointSize = 0.15; // Размер точек

            double scaleX = 2.0; // Увеличить масштаб по X в 2 раза
            double scaleY = 2.0; // Увеличить масштаб по Y в 1.5 раза
            ...
            // Создание модели для нескольких точек
            MeshBuilder pointsBuilder = new MeshBuilder();
            if (Points != null)
            {
                int colorIndex = 0;
                foreach (var point in Points)
                {
                    // Используем разные цвета для разных точек
                    Color pointColor;
                    switch (colorIndex % 4)
                    {
                        case 0: pointColor = Colors.Red; break;
                        case 1: pointColor = Colors.Green; break;
                        case 2: pointColor = Colors.Blue; break;
                        case 3: pointColor = Colors.Yellow; break;
                        default: pointColor = Colors.White; break;
                    }

                    var material = MaterialHelper.CreateMaterial(new SolidColorBrush(pointColor));
                    pointsBuilder.AddEllipsoid(point, pointSize, pointSize, pointSize, 10, 10);
                    colorIndex++;
                }
            }

            // Создание модели для точек
            var pointsModel = new GeometryModel3D(pointsBuilder.ToMesh(), Materials.Red);
            ...
            zLabel.Text = "Z Axis";
            zLabel.Position = new Point3D(minX - 5 * axesOffset, maxY + 5 * axesOffset, 0);
            ...
            newModelGroup.Children.Add(pointsModel); // Добавляем модель с точками
